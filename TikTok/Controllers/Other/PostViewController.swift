...
class PostViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        // add videoView dau tien
        // neu ko add thang nay dau tien thi khi videoView dung sub layer de chay video
        // cac buttons se bi che
    ...
    private func configureVideo(){
        // download the url of the post and use it to play video
        StorageManager.shared.getDownloadURL(for: model) { [weak self] result in
            DispatchQueue.main.async {
                guard let strongSelf = self else {return}
                // stop spinner if we got result back
                ...
                case .failure:
                    // runs the harrypotter video if fail
